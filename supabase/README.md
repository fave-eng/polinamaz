...
## 5. Одно уведомление после полной подготовки урока
...
Уведомление **не запускается автоматически при загрузке файлов**. Поэтому урок можно собирать по частям:

1. загрузить домашнюю работу;
2. загрузить словарь;
3. загрузить грамматику;
4. проверить сайт;
5. открыть **Actions → 2 - Send lesson notification → Run workflow**;
6. указать `lesson_id`, например `lesson-2`.

После первой успешной отправки этот урок считается опубликованным для Telegram. Повторный запуск с тем же `lesson_id` не отправит второе сообщение. Поле `notificationVersion` в JSON урока больше не используется.
